...
struct PenlightNearAnimation
{
    ...
    public static PenlightNearAnimation Default()
      => new PenlightNearAnimation()
      {
    ...
    public float4x4 GetStickMatrix
      (float2 pos, float4x4 xform, float time, uint seed, float volume, float phase, int2 block, int2 seat)
    {
        ...
        //float wavedelta = math.sin(2f*math.PI*1f*(time - math.distance(pos, new float2(0, 0))/2f)); //[-1,1]
        origin.y = (block.y - 1f) * 0.5f + seat.y * 0.02f + rand.NextFloat(-0.2f, 0.2f) + noisedVolume * 0.4f;

        // Swing angle
        var angle = math.cos(phase) * math.lerp(0.2f,1.0f,normalizedVolume);
...
[BurstCompile]
struct PenlightNearAnimationJob : IJobParallelFor
{
    // Input
    public PenlightNearAnimation config;
    ...
    public void Execute(int i)
    {
        ...
        matrices[i] = config.GetStickMatrix(pos, xform, time, seed++,volume,phase,block,seat);
